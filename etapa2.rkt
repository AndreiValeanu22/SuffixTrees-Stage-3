...
; Aceasta filtrează o listă de cuvinte și returnează doar acele cuvinte care încep cu caracterul ch. Dacă un cuvânt este vid, acesta nu va fi inclus în rezultat.
...
; Aceasta primește o listă de sufixe care încep cu același caracter și calculează perechea formată din eticheta AST pentru aceste sufixe și lista noilor sufixe.
; Eticheta este chiar primul caracter, iar noile sufixe se obțin prin eliminarea acestui caracter din sufixele vechi.
...
(define (cst-func suffixes)
    (foldl (λ (cuvant_lista acc)
             (let* ([sufix (longest-common-prefix-of-list suffixes) ]
                    [prefix_cuv (longest-common-prefix cuvant_lista sufix)])
             (if (null? acc)
                 (append (list sufix) (list (car (cdr  prefix_cuv))))
                 (append  acc (list (car (cdr prefix_cuv)))))))
         '() suffixes))
...
(define (suffixes->st labeling-func suffixes alphabet)
   (map (λ (arr) (if (null? (cdr arr))
                     arr
                     (cons (car arr) (suffixes->st labeling-func (cdr arr) alphabet))))
        (filter (λ (list)
                  (not (null? list)))
                (map (λ (caracter)
                       (labeling-func(get-ch-words suffixes caracter)))
                     alphabet))))
...
(define (text->st functie_etichete)
  (λ (text)
    ...
    (suffixes->st functie_etichete (get-suffixes text_$) (sort (remove-duplicates text_$) char<?)))))
